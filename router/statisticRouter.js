...
router.get("/demography", isUser, role("admin"), async (req, res) => {
  try {
    const queries = {
      provinsi: `
...
router.get("/data-dashboard", isUser, role("admin"), async (req, res) => {
  try {
...
router.get("/pembayaran", isUser, role("admin"), async (req, res) => {
  try {
    ...
    const query = `
      SELECT nominal, createdAt 
      FROM pembayaran 
      WHERE createdAt >= NOW() - INTERVAL '${maxDays} days'
