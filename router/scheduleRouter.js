...
router.get(
  ...
  async (req, res) => {
    try {
      const data = await client.query(`SELECT * FROM jadwal ORDER BY id ASC`);

      const jadwal = data.rows;
...
router.post("/simpan", isUser, role("admin"), async (req, res) => {
  try {
...
router.get("/:id", isUser, role("admin", "user"), async (req, res) => {
  try {
...
router.delete("/hapus/:id", isUser, role("admin"), async (req, res) => {
  try {
...
// User
router.post("/simpan-jadwal-user", isUser, role("user"), async (req, res) => {
  try {
    ...
    const existingSchedule = await client.query(
      `SELECT * FROM penjadwalan WHERE user_id = $1 AND jenis = $2`,
      [user_id, type]
    );
    ...
    await client.query(
      `INSERT INTO penjadwalan(user_id, jadwal_id, kegiatan, waktu, mode, jenis)
       VALUES($1, $2, $3, $4, $5, $6)`,
      [user_id, schedule_id, activity, time, mode, type]
    );

    // Kurangi kuota jadwal setelah penjadwalan berhasil
...
router.get(
  "/tampilkan-jadwal/:user_id",
  isUser,
  role("user", "admin"),
  async (req, res) => {
    try {
      const data = await client.query(
        `SELECT * FROM penjadwalan WHERE user_id = $1`,
        [req.params.user_id]
      );
  ...
  async (req, res) => {
    try {
      ...
      const result = await client.query(
        `DELETE FROM penjadwalan WHERE user_id = $1 AND jadwal_id = $2 RETURNING *`,
        [user_id, jadwal_id]
      );

      if (result.rowCount === 0) {
        return res.status(404).json({ message: "Penjadwalan tidak ditemukan" });
      }
